...
import { GET_ERRORS, TEST_DISPATCH } from './types';
...
// Contact
// export const newContact = contactData => dispatch => {  // `dispatch` is part of thunk
export const newContact = contactData => { 
	// // dispatch to the reducer (must have a type)
	

	// axios
	// 	.post('/api/users/signup', contactData)  // `contactData` as passed in from above. we'll hit this api
	// 	.then(res => console.log(res.data))  // success = redirect to a different page
	// 	.catch(err => 
	// 		dispatch({  // call `dispatch` from above since this is an async call thank to thunk
	// 			type: GET_ERRORS,
